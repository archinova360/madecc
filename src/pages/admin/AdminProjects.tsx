...
const API_KEY = 
  (typeof process !== 'undefined' && (process as any).env ? (process as any).env.GOOGLE_MAPS_PLATFORM_KEY : null) || 
...
export default function AdminProjects() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('on-site-network-toggle', loadProjects);
    return () => window.removeEventListener('on-site-network-toggle', loadProjects);
  }, []);

  // Sync back to server whenever projects change
  useEffect(() => {
    ...
    const syncProjects = async () => {
      ...
      const forceOffline = localStorage.getItem('madecc_force_offline') === 'true';
      if (forceOffline) {
        console.log("Offline mode: queued project updates locally.");
        return;
      }
      ...
      try {
        ...
        console.error("Local project updates queued for next uplink sync:", e);
      }
    };

    const timeout = setTimeout(syncProjects, 1000);
  ...
  const handleCreateProject = (e: React.FormEvent) => {
    ...
    const successProject: Project = {
      ...
      location: newProject.location || 'Yaoundé, Cameroun',
      status: 'Planning',
      budget: Number(newProject.budget),
      spent: 0,
      deadline: newProject.deadline,
      progress: 0,
      coordinates: { lat: 3.8667, lng: 11.5167 }, // default Yaoundé coordinates
      milestones: []
    };

    setProjects([successProject, ...projects]);
    console.log('Project configuration deployed to system.');
    ...
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          {loadedFromCache && (
            <span className="px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-widest bg-orange-600/10 text-orange-400 border border-orange-500/20 shadow-sm shadow-orange-600/10">
              Offline Cache-View
            </span>
          )}
        </div>
        ...
        <div className="h-[600px] bg-black/40 border border-gray-800 rounded-3xl overflow-hidden relative">
          {!hasValidKey ? (
            ...
            <APIProvider apiKey={API_KEY} version="weekly">
              <Map
                mapId="bf506934c96d5d0f" // Standard demo Map ID or custom one
              ...
              <div className="absolute top-4 right-4 z-40">
                <button 
                  onClick={() => console.log("Map Diagnostics: If you see a 'BillingNotEnabledMapError', ensure your Cloud Project has a linked billing account. Map ID is set to dynamic fallback.")}
